...
InvoiceService.prototype.lines = function(invoice, cb) { //cb(err, invoice)
    var self = this;
    
    var actions = function(invoice, cb) {
        ...
        console.log('invoice outside', this, this.cb, this.data)
    }

    actions.add = function() {
        console.log('invoice inside', this, this.cb, this.data)
        utils.callAPI(this.data, this.options, function(err, result) {
            if (err) {
                this.cb(err, null);
            }
            else {
                self.get(this.invoice.invoice_id, this.cb);
            }
        })
    };

    return actions;
    // self.add = utils.callAPI(data, options, function(err, result) {
    //     if (err) {
    //         cb(err, null);
    //     }
    //     else {
